...
do_map([{Field, SchemaFun} | More], Namespace, Conf, Acc, SchemaModule) ->
    AbsField = Namespace ++ "." ++ str(Field),
...
add_default_validator(Validators, Struct, Schema) when is_list(Struct) ->
    Type = {'$type_refl', #{ check => fun(V) -> ref_checker(V, Schema:fields(Struct), Schema) end
                           , name => "struct(" ++ Struct ++ ")"
                           }},
    add_default_validator(Validators, Type, Schema);
...
ref_checker(Value, Fields, SchemaModule) ->
    try
        {M, _} = do_map(Fields, "", #{value => Value}, [], SchemaModule),
        find_error(M),
        true
    catch
        _:_ -> false
