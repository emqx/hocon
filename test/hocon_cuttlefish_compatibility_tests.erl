...
cluster_test_() ->
    ...
    , ?_assertEqualPlist(sub(ekka, load_cuttlefish(tp("cluster_mcast.cuttlefish.conf"))),
                       sub(ekka, load_hocon(tp("cluster_mcast.conf")))) % needs quotation
...
zone_test_() ->
    [ ?_assertEqualPlist(sub(external, sub(zones, sub(emqx, load_cuttlefish(tp("zone_external.cuttlefish.conf"))))),
                       sub(external, sub(zones, sub(emqx, load_hocon(tp("zone_external.conf"))))))
    , ?_assertEqualPlist(sub(internal, sub(zones, sub(emqx, load_cuttlefish(tp("zone_internal.cuttlefish.conf"))))),
                       sub(internal, sub(zones, sub(emqx, load_hocon(tp("zone_internal.conf"))))))
    ].
