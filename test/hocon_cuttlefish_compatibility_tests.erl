...
default_test_() ->
    [
        ?_assertEqualPlist(
            load_cuttlefish(tp("default.conf")),
...
cluster_test_() ->
    [
        ?_assertEqualPlist(
            sub(ekka, load_cuttlefish(tp("cluster_manual.cuttlefish.conf"))),
            sub(ekka, load_hocon(tp("cluster_manual.cuttlefish.conf")))
        ),
        ?_assertEqualPlist(
            sub(ekka, load_cuttlefish(tp("cluster_static.cuttlefish.conf"))),
            sub(ekka, load_hocon(tp("cluster_static.conf")))
        ),
        ?_assertEqualPlist(
            sub(ekka, load_cuttlefish(tp("cluster_mcast.cuttlefish.conf"))),
            sub(ekka, load_hocon(tp("cluster_mcast.conf")))
        ),
        ?_assertEqualPlist(
            sub(ekka, load_cuttlefish(tp("cluster_dns.cuttlefish.conf"))),
            sub(ekka, load_hocon(tp("cluster_dns.cuttlefish.conf")))
        ),
        ?_assertEqualPlist(
            sub(ekka, load_cuttlefish(tp("cluster_etcd.cuttlefish.conf"))),
            sub(ekka, load_hocon(tp("cluster_etcd.conf")))
        ),
        ?_assertEqualPlist(
            sub(ekka, load_cuttlefish(tp("cluster_k8s.cuttlefish.conf"))),
...
node_test_() ->
    [
        ?_assertEqualPlist(
            sub(vm_args, load_cuttlefish(tp("node.cuttlefish.conf"))),
            sub(vm_args, load_hocon(tp("node.conf")))
        ),
        ?_assertEqualPlist(
            sub(kernel, load_cuttlefish(tp("node.cuttlefish.conf"))),
            sub(kernel, load_hocon(tp("node.conf")))
        ),
        ?_assertEqualPlist(
            sub(emqx, load_cuttlefish(tp("node.cuttlefish.conf"))),
...
rpc_test_() ->
    [
        ?_assertEqualPlist(
            sub(emqx, load_cuttlefish(tp("rpc.conf"))),
            sub(emqx, load_hocon(tp("rpc.conf")))
        ),
        ?_assertEqualPlist(
            sub(gen_rpc, load_cuttlefish(tp("rpc.conf"))),
...
log_test_() ->
    [
        ?_assertEqualPlist(
            sub(kernel, load_cuttlefish(tp("logger.cuttlefish.conf"))),
            sub(kernel, load_hocon(tp("logger.conf")))
        ),
        ?_assertEqualPlist(
            sub(lager, load_cuttlefish(tp("logger.cuttlefish.conf"))),
            sub(lager, load_hocon(tp("logger.conf")))
        ),
        ?_assertEqualPlist(
            load_cuttlefish(tp("logger_disabled.conf")),
...
acl_test_() ->
    [
        ?_assertEqualPlist(
            sub(emqx, load_cuttlefish(tp("acl.cuttlefish.conf"))),
...
mqtt_test_() ->
    [
        ?_assertEqualPlist(
            sub(emqx, load_cuttlefish(tp("mqtt.conf"))),
...
zone_test_() ->
    ...
    [
        ?_assertEqualPlist(
            Zone(external, load_cuttlefish(tp("zone_external.cuttlefish.conf"))),
            Zone(external, load_hocon(tp("zone_external.conf")))
        ),
        ?_assertEqualPlist(
            Zone(internal, load_cuttlefish(tp("zone_internal.cuttlefish.conf"))),
...
listener_test_() ->
    Listener = fun(Proto, Name, Conf) ->
        hd([
            L
         || L <- sub(listeners, sub(emqx, Conf)),
            maps:get(name, L) =:= Name,
            maps:get(proto, L) =:= Proto
        ])
    end,
    [
        ?_assertEqual(
            Listener(tcp, "external", load_cuttlefish(tp("listener.cuttlefish.conf"))),
            Listener(tcp, "external", load_hocon(tp("listener.conf")))
        ),
        ?_assertEqual(
            Listener(tcp, "internal", load_cuttlefish(tp("listener.cuttlefish.conf"))),
            Listener(tcp, "internal", load_hocon(tp("listener.conf")))
        ),
        ?_assertEqual(
            Listener(ssl, "external", load_cuttlefish(tp("listener.cuttlefish.conf"))),
            Listener(ssl, "external", load_hocon(tp("listener.conf")))
        ),
        ?_assertEqual(
            Listener(ws, "external", load_cuttlefish(tp("listener.cuttlefish.conf"))),
            Listener(ws, "external", load_hocon(tp("listener.conf")))
        ),
        ?_assertEqual(
            Listener(wss, "external", load_cuttlefish(tp("listener.cuttlefish.conf"))),
...
misc_test_() ->
    [
        ?_assertEqualPlist(
            sub(emqx, load_cuttlefish(tp("misc.cuttlefish.conf"))),
...
ldap_test_() ->
    [
        ...
        ?_assertEqualPlist(
            load_cuttlefish(
                tp("ldap_filters.cuttlefish.conf"),
                ss("emqx_auth_ldap.schema")
            ),
...
load_cuttlefish(File) ->
    cuttlefish_generator:map(
        cuttlefish_schema:files(["sample-schemas/emqx.schema"]),
        cuttlefish_conf:file(File)
    ).

load_cuttlefish(File, Schema) ->
    cuttlefish_generator:map(
        cuttlefish_schema:files([Schema]),
        cuttlefish_conf:file(File)
    ).
