...
array_concat_test_() ->
    ...
    , ?_assertEqual(#{<<"x">> => [1, 2], <<"y">> => [1, 2, 1, 2]},
                    binary("x=[1,2],y=${x}[1,2]"))
    , ?_assertEqual(#{<<"x">> => [1, 2], <<"y">> => [[1, 2], [1, 2]]},
                    binary("x=[1,2],y=[${x},[1,2]]"))
    , ?_assertEqual(#{<<"a">> => #{<<"x">> => [1, 2, <<"str">>, #{<<"a">> => 1}]}},
                    binary("a={x=[1,2,str][{a=1}]}"))
    , ?_assertEqual(#{<<"z">> => [#{<<"a">> => 1, <<"b">> => 1}, #{<<"c">> => 1}]},
...
apply_opts_test_() ->
    [ ?_assertEqual({ok, #{<<"GB">> => 1073741824, <<"KB">> => 1024, <<"MB">> => 1048576,
                           ...
                           <<"hour">> => 3600000, <<"kb">> => 1024,
                           <<"mb">> => 1048576, <<"min">> => 60000,
                 ...
                 hocon:load("etc/convert-sample.conf",
                            #{convert => [duration,
                                          ...
                                          percent,
                                          onoff]}))
    , ?_assertEqual({ok, #{<<"GB">> => ok, <<"KB">> => ok, <<"MB">> => ok,
                           <<"day">> => ok, <<"full">> => ok,
                           <<"gb">> => ok, <<"hour">> => ok, <<"kb">> => ok,
                           <<"mb">> => ok, <<"min">> => ok,
                           <<"notkb">> => ok, <<"off">> => ok, <<"on">> => ok,
                           <<"percent">> => ok, <<"sec">> => ok,
                           <<"x">> => #{<<"kb">> => ok, <<"sec">> => ok}}},
                 hocon:load("etc/convert-sample.conf",
